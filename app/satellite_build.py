"""Build a satellite snapshot (vegetation and surface heat) for the app's area.

    .venv/bin/python scripts/build_satellite.py            # rewrites the bundled snapshot in app/data/
    python -m app.satellite_build --out /path/to/file.json  # used by the app's own two-week refresh
...
resampled to a ~30 m WGS84 grid and written as JSON.
"""
import argparse
import json
import math
...
def search(collection, aoi, days, max_cloud, limit=30):
    ...
    r = httpx.post(STAC, json=body, timeout=90)
    r.raise_for_status()
...
def main():
    ...
    ap.add_argument("--out", help="where to write the snapshot (default: the bundled app/data/satellite.json)")
