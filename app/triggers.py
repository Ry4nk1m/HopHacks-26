...
def build_specs(conn, settings, satellite, cond, now):
    ...
    for f in features_in_aoi(conn, settings, ["cooling_center", "community_space"]):
        info = f["info"] or {}
        closed = info.get("status") == "unusable"
        cutoff = _iso(now - timedelta(days=rules.unusable_recheck_days(info.get("recheck_days")))) if closed else stale_cutoff
        stale = not f["last_verified_at"] or f["last_verified_at"] < cutoff
...
def _apply(conn, spec, now):
    ...
    if row["status"] == "resolved":
        ...
        seen = (spec["context"].get("feature") or {}).get("verified") or {}
        if seen.get("status") == "unusable":
            days = min(days, rules.unusable_recheck_days(seen.get("recheck_days")))
